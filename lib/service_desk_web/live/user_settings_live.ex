defmodule ServiceDeskWeb.UserSettingsLive do
  ...
  def handle_event("update_info", %{"user" => params}, socket) do
    user = socket.assigns.current_user

    case Accounts.update_user_info(user, params) do
      {:ok, applied_user} ->
        info = "Your information has been changed"
        user_form =
          applied_user
          |> Accounts.change_user_info()
          |> to_form()

        {:noreply, socket |> put_flash(:info, info) |> assign(:info_form, user_form)}
